...
class UtilController extends Controller
{
    ...
    // NZ
    public function RegisterNZ(Request $request)
    {
        if (session('register_captcha') != $request->captcha) {
            return redirect()->back()->with('failed','Invalid Captcha.');
        }
        ...
        return redirect('login')->with("name", $user->str_id)->with('password', $request->password);
